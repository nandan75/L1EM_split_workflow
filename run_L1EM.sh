...
mkdir -p "$GOR_DIR"
python3 "${L1EM_CGC_dir}median_template_and_pairs.py" "$bamfile" 0.001 | tee "$WORKDIR/baminfo.txt"

# STEP 6: Expectation maximization
echo 'STEP 6: Expectation maximization'
mkdir -p "$L1EM_DIR"
ls "$GOR_DIR"/*pk2 > "$L1EM_DIR/G_of_R_list.txt"
cp $(ls "$GOR_DIR"/*TE_list.txt | head -1) "$L1EM_DIR/TE_list.txt"
python3 "${L1EM_code_dir}L1EM.py" \
  -g "$L1EM_DIR/G_of_R_list.txt" \
  -l "$L1EM_DIR/TE_list.txt" \
  -t $threads \
  -s $EM_threshold
...
cd "$L1EM_DIR"
python3 "${L1EM_utilities_dir}L1EM_readpairs.py" >> "$WORKDIR/baminfo.txt"
python3 "${L1EM_utilities_dir}report_l1_exp_counts.py" > "$WORKDIR/full_counts.txt"
python3 "${L1EM_utilities_dir}report_l1hs_transcription.py" > "$WORKDIR/l1hs_transcript_counts.txt"
python3 "${L1EM_utilities_dir}filtered_and_normalized_l1hs.py" names_final.pkl X_final.pkl $(head -2 "$WORKDIR/baminfo.txt" | tail -1) $(head -3 "$WORKDIR/baminfo.txt" | tail -1) > "$WORKDIR/filter_L1HS_FPM.txt"

# STEP 8: Clean up
echo 'STEP 8: Clean up'
cp *final.pkl "$WORKDIR/"
